...
class CounterexampleRenderTest {
    ...
    fun `renderKotlin renders no reference to any internal oracle symbol`() {
        ...
        //     CaseStep / CaseDelivery / GeneratedCase / RemoveRecord / GraphGenerator.lower
        //     (all public,
    ...
    // ------------------------------------------- computenet-r38y: gossip reaches the snippet

    /**
     * `CaseScript.deliveries` had no renderer, so even an UNSHRUNK replicated counterexample
     * rendered as `CaseScript(steps = listOf(...))` — a snippet that rebuilds a non-replicated
     * script and fails to reproduce, saying nothing about why.
     *
     * ## What "round-trip" can honestly mean here
     *
     * Not execution. Nothing in this module can invoke the Kotlin compiler on the rendered
     * string — the same limitation `renderKotlin renders no reference to any internal oracle
     * symbol` above records, and `ShrinkerBs14Test` restates ("not through a compiled snippet,
     * which no test in this module can invoke the Kotlin compiler on"). So the check is made at
     * the strongest level that is available rather than quietly weakened: the rendered text is
     * asserted to state **every** delivery of the shrunk case, each as the exact literal that
     * reconstructs that `CaseDelivery` — same `atStep`, same `into`, same `from` — so a renderer
     * that emitted a subset, a wrong index, or a swapped source pair fails here.
     */
    @Test
    fun `computenet-r38y renderKotlin renders a replicated case's gossip deliveries`() {
        val counterexample = shrunkGossipingCounterexample(seed = 1L)
        val deliveries = counterexample.case.script.deliveries
        withClue("the fixture must be replicated, or this test asserts nothing") {
            deliveries.isNotEmpty() shouldBe true
        }

        val rendered = counterexample.renderKotlin()

        withClue(rendered) {
            rendered shouldContain "deliveries = listOf("
            deliveries.forEach { delivery ->
                rendered shouldContain "civictech.oracle.gen.CaseDelivery(" +
                    "atStep = ${delivery.atStep}, " +
                    "into = civictech.oracle.model.SourceId(${literal(delivery.into.id)}), " +
                    "from = civictech.oracle.model.SourceId(${literal(delivery.from.id)}))"
            }
        }
    }

    /**
     * The negative half, and the reason the argument is emitted conditionally: `deliveries`
     * defaults to empty, so a non-replicated snippet says nothing about gossip and stays
     * byte-identical to what it rendered before computenet-r38y.
     */
    @Test
    fun `computenet-r38y a non-replicated case renders no deliveries argument at all`() {
        val rendered = shrunkCounterexample(seed = 107L).renderKotlin()
        withClue(rendered) {
            rendered shouldNotContain "deliveries"
            rendered shouldNotContain "CaseDelivery"
        }
    }
    ...
    /**
     * A shrunk counterexample whose case carries gossip.
     *
     * The deliveries are attached to a drivable two-source topology rather than generated by
     * `GeneratorConfig.replicatedSweep()`, for the reason `ShrinkerTest.gossipingCase` records in
     * full: the sweep's sources are `orMap` cells, which `CaseExecution.scriptSourceFor` refuses
     * to drive on this branch, and the shrinker executes every candidate. A `CaseDelivery` is
     * inert to the drive loop, so this exercises the renderer on exactly the data under test.
     * The `union` fan-in is what keeps both sources — and therefore the gossip — alive through
     * pass 3.
     */
    private fun shrunkGossipingCounterexample(seed: Long): Counterexample {
        // Two set sources fanning in to one `union`: both are read, so pass 3 retires neither
        // and the gossip between them survives the shrink to reach the renderer.
        val config = chainConfig().copy(
            sourceCount = 2,
            terminalCount = 1,
        ...
        withClue("the fixture needs two sources to gossip between") { sources.size shouldBe 2 }
        val gossiping = case.copy(
            script = CaseScript(
                case.script.steps,
                listOf(
                    CaseDelivery(atStep = 0, into = sources[0], from = sources[1]),
                    CaseDelivery(atStep = case.script.steps.size, into = sources[1], from = sources[0]),
                ),
            ),
        )
        val terminal = gossiping.topology.terminals.first().name
        val model = CaseExecution.referenceModelFor(gossiping.topology)
        val reference = Reference { script -> model.eval(script) + (terminal to SENTINEL) }
        return Shrinker.run(gossiping, reference = reference)
    }
