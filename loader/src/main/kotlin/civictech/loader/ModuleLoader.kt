...
class ModuleLoader(
     ...
     *
     * **computenet-iifu — a host whose two callbacks throw the identical
     * [Throwable] instance (a cached/singleton exception, or a shared stub in a
     * test double) does not raise `IllegalArgumentException("Self-suppression not
     * permitted")` here.** That guard belongs to `java.lang.Throwable.addSuppressed`;
     * in Kotlin, `t.addSuppressed(u)` resolves to the stdlib extension
     * `kotlin.ExceptionsKt.addSuppressed`, whose own generated bytecode compares
     * the two references first and returns without ever calling the JDK method
     * when they are identical (confirmed by decompiling this class:
     * `invokestatic kotlin/ExceptionsKt.addSuppressed`, guarded by an `if_acmpeq`
     * on the two arguments). So the host's own throwable always reaches the
     * caller unwrapped, whether or not the second callback's throwable is the
     * same instance as the first — no explicit `t === callbackFailure` guard is
     * needed on top of it. See
     * `ModuleLoadFailureTest`'s "when both post-commit callbacks throw the same
     * throwable instance" test, which locks this in.
     *
     ...
     *
     *    **computenet-iifu, assessed and ruled out (no guard needed).** `removal`
     *    (whatever [unregister] threw) and `restore` (whatever the
     *    [ModuleRegistration.register] call above threw) can never be the same
     *    `Throwable` instance here for two independent reasons. First, the same
     *    reason [load] cites above: Kotlin's `t.addSuppressed(u)` resolves to
     *    `kotlin.ExceptionsKt.addSuppressed`, which already no-ops instead of
     *    throwing `IllegalArgumentException` when the two references are
     *    identical — there is no self-suppression crash to guard against in
     *    Kotlin regardless of the seam. Second, and specific to this seam: the
     *    restore's `ModuleRegistration.register` call is *not* one of [unload]'s
     *    injectable parameters — it is always the real production function, and
     *    it always constructs a fresh [civictech.nature.RegistrationRefusedException]
     *    from its own [civictech.nature.ValidationReport] rather than rethrowing
     *    any throwable handed to it. [unregister] is the only side a caller (or a
     *    test) can substitute, so even a host that deliberately throws a shared,
     *    cached exception from [unregister] cannot make the restore's call throw
     *    that same instance back — `restore` is always a distinct object from
     *    `removal`. `ModuleUnloadTest`'s two [JAR1-UNL-07] tests already exercise
     *    both throwables as genuinely independent instances (one
     *    `IllegalStateException`, the other a
     *    [civictech.nature.RegistrationRefusedException]); no further test is
     *    added for the same-instance case because the public seam cannot produce
     *    it.
     *
