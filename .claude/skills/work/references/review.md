...
- it edits prose that is itself the deliverable. That covers files under `.claude/skills/` or `doc/spec/`, any file the acceptance criteria name, and any task whose deliverable is a document. The exception is a **conforming repair**, defined by its form so that it cannot be argued into: either a **deletion** of text that a rule already written down elsewhere — the document's own header, the acceptance criteria, a decision recorded on the bead — renders redundant or superseded; or an **insertion whose exact content that rule fixes**, meaning a label from a closed vocabulary, a line number, an id, or a pointer to a section that exists. Deleting a byte-identical duplicate line; adding the `MEASURED`/`ESTIMATED` label the document's own evidence rule requires; correcting a citation to the line it names; deleting a paragraph describing a design an amendment already superseded. Quote the governing rule and the text either side of the edit in your verdict; if you cannot quote a rule that decides it, it is not conforming. **No sentence you composed is ever conforming**, however small and however true — not a rewrite, and not new prose either. Content the acceptance demands and the deliverable lacks is an unmet criterion, so FAIL or DRAFT it; authoring it yourself and certifying it is the failure this bullet exists to stop, and is worse than the rewrite case because there is no prior text to compare against. Replacing superseded text with your own description of what superseded it is substantive: delete, and say what is missing.
...
The script prints only the new id. Check that the file holds an id before any later call reads it. If the residual's subject exists only on the feature branch, also run `bd update <new-id> --set-metadata base_branch=feature/<feature-id>`. Otherwise the fix gets cut from `main`, where its subject does not exist yet. Set it only when the subject will NOT reach main soon: the branch you are reviewing usually merges within minutes, and the field then points at spent code without failing loudly. When the blocker is a ready, green PR, record a dependency on it and let the merge clear it instead (computenet-osax: five instances, one a feature branch cut from another feature's branch).
