...
`--expect-classes N` whenever the run carried N `--tests` filters — a filter
matching nothing is dropped SILENTLY when a sibling matches, and re-deriving a
total cannot see it), and the
