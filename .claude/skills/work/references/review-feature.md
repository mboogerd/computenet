...
`--expect-classes N` on any multi-`--tests` run, since a filter that matches
nothing is dropped silently and the total you re-derive is of a run that asked
for less than the command said — the
