...
Each row is `<signature> — what it does`. The signatures are checked against
the scripts by `.claude/skills/work/scripts/usage-table.test.sh`, which `sibling-tests.sh` runs
whenever a script changes, so a row cannot quietly go stale — but a signature
is a summary: the long-optioned ones state their full usage in their own
header.
...
| `next-batch.py` | `<feature-id> [--actor NAME]` — Next set of tasks safe to run in parallel — file-disjoint AND within machine capacity |
...
| `file-friction.sh` | `--type bug\|feature --title T --desc D --accept A [--parent computenet-wpvy] [--priority N] [--skill-version <sha>]` — Files a friction item collision-free under the SDLC epic, open and unclaimed |
...
| `session-holder.sh` | `[--check <token>]` — this session's unique holder token, and `--check <token>` → MINE/LIVE/DEAD/UNKNOWN/FOREIGN; what tells a live sibling from a crash leftover, which `assignee` cannot |
