...
| `bead.sh` | `[-C <dir>] <id> [-r] [jq-filter]` — projected `bd show`: the bead's own fields as one object, `dependencies` dropped (57KB -> 7KB); no `.[0]` unwrap. Output over 25KB is written to a file and the path printed. `-C` is forwarded to `bd`, and is required from a worktree — only the main checkout holds the database |
