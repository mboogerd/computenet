...
- **Membership convergence race remains for entirely-unknown joiners.** The creation fence (R13) turns a *known-but-not-yet-gossiped* covering member into a conservative hold, and DEGRADE handles a *recoverable* stall, but a covering member the local view hasn't learned about at all can still release a wave early in a narrow window. This is the eventual-membership race, not a per-wave settlement bug.
