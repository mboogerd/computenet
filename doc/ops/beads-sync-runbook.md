...
| `.claude/skills/remediate-friction/SKILL.md` (§1 Start pull; §6 Finalize publish) | pull + push | Once at session start, then bracketed around each item claimed or filed, then once at session close — same acquisition-brackets-writes policy as `/work`, on its own separate lane |
