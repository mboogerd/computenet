...
| `:identity` | JDK-only Ed25519 keypairs, a fail-closed file-backed key store with machine-distinguishable refusal reasons (`WORLD_READABLE`, `MALFORMED`, `UNSUPPORTED`, `KEYPAIR_MISMATCH`, `INCOMPLETE_PAIR`, `NO_POSIX_PERMISSIONS`), and key-derived `KeyId` fingerprints; a peer's `PeerId` resolves through the kernel's `PeerIdentityBinding` seam (DSC4 interim, computenet-376c); implements the kernel's `SignatureVerifier` seam (DSC1, epic `computenet-ssa`). `civictech.identity.anchor` (DSC4, epic `computenet-5y8t`) is the anchor-vouched binding over that seam: `AnchorIssuer` signs `IdentityStatement`s (canonical bytes, domain-tagged) binding a stable name to a current key; `AnchorVouchedBinding` verifies them offline against a configured `acceptedIssuers` map; `AcceptedIssuerStore` loads public issuer halves from files and refuses a private key found there. JDK-only — no third-party crypto. | `api(:kernel)`; `:kernel` must not depend on it |
...
| `:timetravel` | Offline, headless time-travel tooling over durability journals — reader, reconstruction, diff, and a CLI (TTD1, epic `computenet-ocv`). Consumes the kernel's read-side seam (`JournalRecords.decode` / `scanJournalFile`). A leaf on `:kernel` by design (epic §2 "Module"); nothing depends on it until TTD2 adds `:inspect -> :timetravel`. Must not depend on `:inspect`, `:wire`, `:concord`, `:identity`, `:iroh`, `:oracle`, `:query` or any `:demo:*` module; its own `ModuleDependencyTest` enforces that against both the build file (both directions) and the runtime classpath ([TTD1-53]). | `:kernel`, kotlinx.serialization |
