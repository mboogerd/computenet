...
## Milestone 1 — One kernel (consolidation)

*Goal: a single, coherent codebase that matches spec sections 10 and 31.*

1. Fold `ManagedRunner` into `ManagedHost` (C-2); add the `inline`/simulated
   host configuration and use it to replace `Thread.sleep` tests (C-8 fix:
   sequence-number FIFO tiebreaker + deterministic tests).
...
*Exit criterion: kernel tests green with no sleeps, no reflection in port
resolution, one host class.*

## Milestone 2 — Context and links (the semant   ic spine)
...
*Exit criterion: a fork-join demo provably glitch-free under randomized
scheduling (invariant-checked, not example-checked — pulls M4 forward a bit).*
